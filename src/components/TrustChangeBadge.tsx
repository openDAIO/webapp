interface TrustChangeBadgeProps {
  before: number;
  after: number;
}

export default function TrustChangeBadge({ before, after }: TrustChangeBadgeProps) {
  ...
  const label = change > 0 ? `+${change} Trust` : change < 0 ? `${change} Trust` : 'No Change';
