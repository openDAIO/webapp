...
export default function TokenFlowBadge({ amount, asset = 'TOK' }: TokenFlowBadgeProps) {
  const tone =
    amount > 0
  ...
  const label = amount > 0 ? `+${amount.toFixed(1)} ${asset}` : amount < 0 ? `${amount.toFixed(1)} ${asset}` : `0 ${asset}`;
